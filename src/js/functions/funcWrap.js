export class MathFunctions {
  ...
  exponentiation(base, exponent) {
    ...
    if (!Number.isInteger(exponent)) {
      ...
      else if (base !== Math.E) exponent = exponent * this.naturalLog(base);
  ...
  hyperbolicSine(x) {
    let result =
      (this.naturalExponentiation(x) - this.naturalExponentiation(-x)) / 2;
    return parseFloat(result);
  }
  ...
  squareRoot(num) {
    ...
    while (diff > precisionX) {
      ...
      if (diff < 0) {
        //handle the cases of decimal inputs
        diff *= -1;
      }
